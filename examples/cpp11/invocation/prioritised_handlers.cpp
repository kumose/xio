...
class handler_priority_queue : public xio::execution_context
{
public:
  ...
  xio::executor_binder<Handler, executor>
  wrap(int priority, Handler handler)
  {
    return xio::bind_executor(
...
int main()
{
  ...
  xio::post(io_context, pri_queue.wrap(0, low_priority_handler()));

  // Start an asynchronous accept that will complete immediately.
  tcp::endpoint endpoint(xio::ip::address_v4::loopback(), 0);
  ...
  xio::steady_timer timer(io_context);
  timer.expires_at(xio::steady_timer::clock_type::time_point::min());
