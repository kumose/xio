...
using tcp_socket = xio::basic_stream_socket<
    tcp, xio::io_context::executor_type>;
...
public:
  ...
  {
    xio::io_context& io_context = xio::query(
        token.socket_.get_executor(), xio::execution::context);
...
int main(int argc, char* argv[])
{
  try
  {
    ...
    xio::async_write(socket, xio::buffer(msg),
        close_after(std::chrono::seconds(10), socket));

    for (std::string input_buffer;;)
    {
      // Run an asynchronous read operation with a timeout.
      std::size_t n = xio::async_read_until(socket,
          xio::dynamic_buffer(input_buffer), '\n',
          close_after(std::chrono::seconds(10), socket));
