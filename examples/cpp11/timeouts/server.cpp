...
private:
  ...
  void read_line()
  {
    ...
    xio::async_read_until(socket_,
        xio::dynamic_buffer(input_buffer_), '\n',
  ...
  void write_line()
  {
    ...
    xio::async_write(socket_,
        xio::buffer(output_queue_.front()),
        [this, self](const std::error_code& error, std::size_t /*n*/)
        {
          // Check if the session was stopped while the operation was pending.
          if (stopped())
...
{
public:
  udp_broadcaster(xio::io_context& io_context,
...
private:
  ...
  {
    std::error_code ignored_error;
    socket_.send(xio::buffer(msg), 0, ignored_error);
  }
...
int main(int argc, char* argv[])
{
  try
  {
    ...
    udp::endpoint broadcast_endpoint(
        xio::ip::make_address(argv[2]), atoi(argv[3]));
