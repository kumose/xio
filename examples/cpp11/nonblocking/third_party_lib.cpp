...
public:
  ...
  {
    if (std::size_t len = socket_.read_some(xio::buffer(data_), ec))
    {
      write_buffer_ = xio::buffer(data_, len);
      state_ = writing;
    }
  }
  ...
  void do_write(std::error_code& ec)
  {
    if (std::size_t len = socket_.write_some(
          xio::buffer(write_buffer_), ec))
    {
      write_buffer_ = write_buffer_ + len;
      state_ = xio::buffer_size(write_buffer_) > 0 ? writing : reading;
    }
  }
...
private:
  ...
  xio::const_buffer write_buffer_;
};
    ...
    if (session_impl_.want_read() && !read_in_progress_)
    {
      ...
      socket_.async_wait(tcp::socket::wait_read,
          [this, self](std::error_code ec)
          {
            ...
            // wants.
            if (!ec || ec == xio::error::would_block)
              do_operations();
