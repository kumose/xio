...
typedef std::shared_ptr<xio::steady_timer> timer_ptr;
...
public:
      ...
      next_frame_number_(1)
  {
    ...
    acceptor_.async_accept(*new_socket,
        std::bind(&server::handle_accept, this,
          xio::placeholders::error, new_socket));
  ...
  void handle_accept(const std::error_code& ec, tcp_socket_ptr socket)
  {
    if (!ec)
    {
      ...
      xio::async_read(*socket, request->to_buffers(),
          std::bind(&server::handle_control_request, this,
            xio::placeholders::error, socket, request));
    }
      ...
      tcp_socket_ptr socket, control_request_ptr request)
  {
    if (!ec)
    {
      ...
      timer_ptr delay_timer(
          new xio::steady_timer(acceptor_.get_executor()));
      delay_timer->expires_after(xio::chrono::seconds(2));
