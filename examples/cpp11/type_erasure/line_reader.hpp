...
private:
  virtual void async_read_line_impl(std::string prompt,
      xio::any_completion_handler<void(std::error_code, std::string)> handler) = 0;
...
public:
  ...
  auto async_read_line(std::string prompt, CompletionToken&& token)
    -> decltype(
        xio::async_initiate<CompletionToken, void(std::error_code, std::string)>(
          initiate_read_line(), token, this, prompt))
  {
    return xio::async_initiate<CompletionToken, void(std::error_code, std::string)>(
        initiate_read_line(), token, this, prompt);
  }
};
