...
{
public:
  explicit stdin_line_reader(xio::any_io_executor ex);

private:
  void async_read_line_impl(std::string prompt,
      xio::any_completion_handler<void(std::error_code, std::string)> handler) override;

  xio::posix::stream_descriptor stdin_;
  std::string buffer_;
};
