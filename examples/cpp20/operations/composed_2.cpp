...
template <
  xio::completion_token_for<void(std::error_code, std::size_t)>
    CompletionToken>
auto async_write_message(tcp::socket& socket,
  ...
  // However, when the completion token is xio::yield_context (used for
  // stackful coroutines) the return type would be std::size_t, and when the
  // completion token is xio::use_future it would be std::future<std::size_t>.
  // When the completion token is xio::deferred, the return type differs for
  ...
  // the return type of xio::async_initiate.
{
  ...
  // straight through.)
  auto initiation = [](
      xio::completion_handler_for<void(std::error_code, std::size_t)>
