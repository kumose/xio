...
auto async_write_messages(tcp::socket& socket,
  ...
  // the return type of xio::async_compose.
{
  ...
  // The xio::async_compose function takes:
  //
  ...
  // provided for us by the xio::async_compose function, and takes care
      ...
      ) mutable
      {
        reenter (coro)
        {
          while (repeat_count > 0)
          {
            ...
            yield xio::async_write(socket,
                xio::buffer(*encoded_message), std::move(self));
            if (error)
              break;
          }
