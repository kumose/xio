  ...
  // the return type of xio::async_initiate.
{
      ...
      const char* message)
  {
    ...
    auto executor = xio::get_associated_executor(
        completion_handler, socket.get_executor());

    // ... and then binding this executor to our adapted completion handler
    // using the xio::bind_executor function.
    std::size_t length = std::strlen(message);
    if (length == 0)
    {
      xio::post(
          xio::bind_executor(executor,
            std::bind(std::forward<decltype(completion_handler)>(
                completion_handler), xio::error::invalid_argument)));
    }
    else
    {
      xio::async_write(socket,
          xio::buffer(message, length),
          xio::bind_executor(executor,
            std::bind(std::forward<decltype(completion_handler)>(
...
void test_deferred()
{
  ...
  xio::async_operation auto op =
    async_write_message(socket, "", xio::deferred);
