...
// NOTE: This example requires the new xio::async_initiate function. For
...
template <typename T,
  xio::completion_token_for<void(std::error_code)> CompletionToken>
auto async_write_messages(tcp::socket& socket,
  ...
  // void. In this example, when the completion token is xio::yield_context
  // (used for stackful coroutines) the return type would also be void, as
  // there is no non-error argument to the completion handler. When the
  // completion token is xio::use_future it would be std::future<void>. When
  // the completion token is xio::deferred, the return type differs for each
  // asynchronous operation.
  //
  // In C++20 we can omit the return type as it is automatically deduced from
  // the return type of xio::async_initiate.
{
  ...
  // straight through.)
  auto initiation = [](
      xio::completion_handler_for<void(std::error_code)>
        auto&& completion_handler,
      tcp::socket& socket,
      std::unique_ptr<std::string> encoded_message,
      std::size_t repeat_count,
      std::unique_ptr<xio::steady_timer> delay_timer)
  {
    ...
    struct intermediate_completion_handler
    {
      ...
      std::unique_ptr<xio::steady_timer> delay_timer_;
      ...
      xio::executor_work_guard<tcp::socket::executor_type> io_work_;
      ...
      void operator()(const std::error_code& error, std::size_t = 0)
      {
        if (!error)
        {
          ...
          case waiting:
            state_ = writing;
            xio::async_write(socket_,
                xio::buffer(*encoded_message_), std::move(*this));
            return; // Composed operation not yet complete.
          }
        }
      ...
      // handler does not have its own.
      using allocator_type = xio::associated_allocator_t<
          typename std::decay<decltype(completion_handler)>::type,
          std::allocator<void>>;

      allocator_type get_allocator() const noexcept
      {
        return xio::get_associated_allocator(
    ...
    // completion handler.
    auto encoded_message_buffer = xio::buffer(*encoded_message);
    xio::async_write(socket, encoded_message_buffer,
        intermediate_completion_handler{
          socket, std::move(encoded_message),
          repeat_count, std::move(delay_timer),
          intermediate_completion_handler::starting,
          xio::make_work_guard(socket.get_executor()),
  ...
  std::unique_ptr<xio::steady_timer> delay_timer(
      new xio::steady_timer(socket.get_executor()));

  // The xio::async_initiate function takes:
  //
...
void test_deferred()
{
  ...
  xio::async_operation auto op = async_write_messages(
