...
template <xio::completion_handler_for<void(std::string)> Handler>
class read_input_state
{
public:
  read_input_state(Handler&& handler)
    : handler_(std::move(handler)),
      work_(xio::make_work_guard(handler_))
  {
  }
  ...
  static read_input_state* create(Handler&& handler)
  {
    ...
    struct deleter
    {
      ...
      typename std::allocator_traits<
        xio::associated_allocator_t<Handler,
          xio::recycling_allocator<void>>>::template
    ...
    } d{xio::get_associated_allocator(handler,
          xio::recycling_allocator<void>())};
  ...
  static void callback(void* arg, const char* result)
  {
    ...
    } d{xio::get_associated_allocator(self->handler_,
          xio::recycling_allocator<void>())};
    ...
    xio::dispatch(state.work_.get_executor(),
        xio::bind_allocator(d.alloc,
          [
...
private:
  ...
  xio::executor_work_guard<
    xio::associated_executor_t<Handler>> work_;
};

// The initiating function for the asynchronous operation.
template <xio::completion_token_for<void(std::string)> CompletionToken>
auto async_read_input(const std::string& prompt, CompletionToken&& token)
{
  ...
  auto init = [](
      xio::completion_handler_for<void(std::string)> auto handler,
  ...
  // such as a future.
  return xio::async_initiate<CompletionToken, void(std::string)>(
...
void test_deferred()
{
  ...
  auto op = async_read_input("Enter your name", xio::deferred);
...
void test_future()
{
  ...
  std::future<std::string> f =
    async_read_input("Enter your name", xio::use_future);
