...
int main()
{
  ...
  auto prefer_low = xio::prefer(ex, custom_props::low_priority);
  auto low = xio::require(ex, custom_props::low_priority);
  auto med = xio::require(ex, custom_props::normal_priority);
  auto high = xio::require(ex, custom_props::high_priority);
  ...
  xio::require(ex, custom_props::priority{-1}).execute([&]{ sched.stop(); });
  sched.run();
  std::cout << "polymorphic query result = " << xio::query(poly_high, custom_props::priority{}) << "\n";
}
