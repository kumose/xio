...
struct async_write_messages_implementation
{
  ...
  // completion handler is provided for us by the xio::async_compose
      ...
      std::size_t = 0)
  {
    reenter (coro_)
    {
      while (repeat_count_ > 0)
      {
        ...
        yield xio::async_write(socket_,
            xio::buffer(*encoded_message_), std::move(self));
        if (error)
          break;
      }
...
auto async_write_messages(tcp::socket& socket,
  ...
  //
  // In C++11 we deduce the type from the call to xio::async_compose.
  -> decltype(
      xio::async_compose<
  ...
  return xio::async_compose<
    CompletionToken, void(std::error_code)>(
      async_write_messages_implementation{socket,
        ...
        std::move(delay_timer), xio::coroutine()},
      token, socket);
}
