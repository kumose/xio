...
private:
  awaitable<void> reader()
  {
    try
    {
      ...
      {
        std::size_t n = co_await xio::async_read_until(socket_,
            xio::dynamic_buffer(read_msg, 1024), "\n", use_awaitable);
  ...
  awaitable<void> writer()
  {
    try
    {
      while (socket_.is_open())
      {
        ...
        {
          co_await xio::async_write(socket_,
              xio::buffer(write_msgs_.front()), use_awaitable);
