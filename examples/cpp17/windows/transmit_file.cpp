...
using xio::ip::tcp;
using xio::windows::overlapped_ptr;
using xio::windows::random_access_handle;

typedef xio::basic_stream_socket<tcp,
    xio::io_context::executor_type> tcp_socket;

typedef xio::basic_socket_acceptor<tcp,
    xio::io_context::executor_type> tcp_acceptor;
...
public:
  ...
  static pointer create(xio::io_context& io_context,
...
private:
  connection(xio::io_context& io_context, const std::string& filename)
    : socket_(io_context),
      filename_(filename),
      file_(io_context)
  {
...
{
public:
  server(xio::io_context& io_context,
...
#else // defined(XIO_HAS_WINDOWS_OVERLAPPED_PTR)
