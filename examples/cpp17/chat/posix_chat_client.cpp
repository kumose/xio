...
{
public:
  posix_chat_client(xio::io_context& io_context,
...
private:
  ...
  void do_read_header()
  {
    xio::async_read(socket_,
        xio::buffer(read_msg_.data(), chat_message::header_length),
  ...
  {
    xio::async_read(socket_,
        xio::buffer(read_msg_.body(), read_msg_.body_length()),
        [this](std::error_code ec, std::size_t /*length*/)
        {
          if (!ec)
          {
  ...
  void do_write_output()
  {
    ...
    std::array<xio::const_buffer, 2> buffers = {{
      xio::buffer(read_msg_.body(), read_msg_.body_length()),
      xio::buffer(eol) }};
    xio::async_write(output_, buffers,
        [this](std::error_code ec, std::size_t /*length*/)
        {
          if (!ec)
          {
  ...
  void do_read_input()
  {
    ...
    xio::async_read_until(input_, input_buffer_, '\n',
        [this](std::error_code ec, std::size_t length)
        {
          if (!ec)
          {
          ...
          else if (ec == xio::error::not_found)
          {
  ...
  void do_write_message()
  {
    xio::async_write(socket_,
        xio::buffer(write_msg_.data(), write_msg_.length()),
        [this](std::error_code ec, std::size_t /*length*/)
        {
          if (!ec)
          {
