...
using xio::local::stream_protocol;
...
private:
  ...
  {
    xio::async_write(socket_, xio::buffer(data_, size),
        [this](std::error_code ec, std::size_t /*size*/)
        {
          if (!ec)
          {
...
int main()
{
  try
  {
    ...
    xio::local::connect_pair(socket, filter_socket);
    ...
    for (;;)
    {
      ...
      xio::write(socket, xio::buffer(request));

      // Wait for reply from filter.
      std::vector<char> reply(request.size());
      xio::read(socket, xio::buffer(reply));
...
#else // defined(XIO_HAS_LOCAL_SOCKETS)
