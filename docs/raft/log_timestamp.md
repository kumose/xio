...
It is disabled by default, and you can enable it by setting `replicate_log_timestamp_` in [`xio_options`](../include/libnuraft/xio_service_options.hxx) to `true`. [`log_entry`](../include/libnuraft/log_entry.hxx) has an attribute `timestamp_us_` and the timestamp (in microseconds) is automatically set by NuRaft. Your log store implementation should properly store the timestamp and be able to restore it when NuRaft wants to retrieve the log entry. [`log_entry`](../include/libnuraft/log_entry.hxx) provides `get_timestamp` and `set_timestamp` APIs to get and set timestamps.
