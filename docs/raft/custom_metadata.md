...
NuRaft provides an ability to ship custom metadata for each message and response that can be used for your own verification. When you set [`xio_options`](../include/libnuraft/xio_service_options.hxx), there are a few options for it:
```C++
xio_service::options xio_opt;
xio_opt.write_req_meta_  = my_write_req_meta;
xio_opt.read_req_meta_   = my_read_req_meta;
xio_opt.write_resp_meta_ = my_write_resp_meta;
xio_opt.read_resp_meta_  = my_read_resp_meta;
```

* `write_req_meta_`: return custom metadata to be shipped with the Raft message.
```c++
std::string my_write_req_meta(const xio_service::meta_cb_params& params) {
...
```c++
bool my_read_req_meta(const xio_service::meta_cb_params& params,
...
```c++
std::string my_write_resp_meta(const xio_service::meta_cb_params& params) {
...
```c++
bool my_read_resp_meta(const xio_service::meta_cb_params& params,
                       const std::string& meta) {
