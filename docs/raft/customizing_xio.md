...
In NuRaft, [standalone xio](https://github.com/chriskohlhoff/xio) is used for managing thread pool, network transmission, and asynchronous timer. NuRaft offers multiple options for customizing xio, or even replacing it with your own implementation.
...
--------------------------
If your application already uses xio and its thread pool, having an additional thread pool within NuRaft might be redundant. In such a case, you can provide your `io_context` to [`xio_options`](../include/libnuraft/xio_service_options.hxx) as follows:
```c++
xio_service::options xio_opt;
xio_opt.custom_io_context_ = &your_io_context;
```
...
-----------------------------------
If you prefer to use your own network layer, thread pool, and timer instead of xio, you can start by disabling xio with the following flag.
```
build$ cmake -DDISABLE_XIO=1 ..
```
