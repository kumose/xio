...
To enable streaming mode, ensure `streaming_mode_` in [`xio_service_options`](../include/libnuraft/xio_service_options.hxx) is set to `true`. Additionally, there are two parameters for operating in streaming mode, in [`raft_params`](../include/libnuraft/raft_params.hxx).
