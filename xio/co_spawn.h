 ...
 * execution.
 *
 * @param a The xio::awaitable object that is the result of calling the
 ...
 * @par Example
 * @code
 * xio::awaitable<std::size_t> echo(tcp::socket socket)
 * {
 ...
 *           xio::buffer(data), xio::use_awaitable);
 *
 *       co_await xio::async_write(socket,
 *           xio::buffer(data, n), xio::use_awaitable);
 *
 *       bytes_transferred += n;
 *     }
 *   }
 ...
 * @par Example
 * @code
 * xio::awaitable<void> echo(tcp::socket socket)
 * {
 *   try
 *   {
 *     char data[1024];
 ...
 * @c xio::awaitable<R,E> that will be used as the coroutine's entry
 * point.
 *
 ...
 * @code xio::awaitable<R, AwaitableExecutor> F() @endcode
 *
 * @par Example
 * @code
 * xio::awaitable<std::size_t> echo(tcp::socket socket)
