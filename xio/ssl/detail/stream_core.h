...
#ifndef XIO_SSL_DETAIL_STREAM_CORE_HPP
#define XIO_SSL_DETAIL_STREAM_CORE_HPP
            ...
            struct stream_core {
                ...
                stream_core(stream_core &&other)
                    : engine_(static_cast<engine &&>(other.engine_)),
                      ...
                      input_(other.input_) {
                    other.input_ = xio::const_buffer(0, 0);
                }
                ...
                xio::steady_timer pending_read_;

                // Timer used for storing queued write operations.
                xio::steady_timer pending_write_;

                // Helper function for obtaining a time value that always fires.
                static xio::steady_timer::time_point neg_infin() {
                    return (xio::steady_timer::time_point::min)();
                }

                // Helper function for obtaining a time value that never fires.
                static xio::steady_timer::time_point pos_infin() {
                    return (xio::steady_timer::time_point::max)();
                }

                // Helper function to get a timer's expiry time.
                static xio::steady_timer::time_point expiry(
                    const xio::steady_timer &timer) {
                    return timer.expiry();
                }

                // A buffer that may be used to prepare output intended for the transport.
                xio::mutable_buffer output_buffer() {
                    return xio::buffer(output_buffer_space_.get(), output_buffer_size_);
                }

                // A buffer that may be used to read input intended for the engine.
                xio::mutable_buffer input_buffer() {
                    return xio::buffer(input_buffer_space_.get(), input_buffer_size_);
                }
                ...
                static std::size_t clamp_buffer_size(std::size_t size) {
                    return size < std::size_t(max_tls_record_size)
                               ? std::size_t(max_tls_record_size)
                               : size;
                }
...
#endif // XIO_SSL_DETAIL_STREAM_CORE_HPP
