        ...
        namespace detail {
            engine::engine(SSL_CTX *context,
                ...
                if (!ssl_) {
                    ...
                    xio::detail::throw_error(ec, "engine");
                }
            ...
            xio::error_code engine::set_verify_mode(
            ...
            xio::error_code engine::set_verify_depth(
            ...
            xio::error_code engine::set_verify_callback(
            ...
            engine::want engine::handshake(
                ...
                return perform((type == xio::ssl::stream_base::client)
                                   ? &engine::do_connect
                                   : &engine::do_accept, 0, 0, ec, 0);
            }

            engine::want engine::shutdown(xio::error_code &ec) {
                return perform(&engine::do_shutdown, 0, 0, ec, 0);
            }

            engine::want engine::write(const xio::const_buffer &data,
            ...
            engine::want engine::read(const xio::mutable_buffer &data,
                                      xio::error_code &ec, std::size_t &bytes_transferred) {
                if (data.size() == 0) {
                    ec = xio::error_code();
                    return engine::want_nothing;
            ...
            xio::mutable_buffer engine::get_output(
                const xio::mutable_buffer &data) {
                int length = ::BIO_read(ext_bio_,
                                        data.data(), static_cast<int>(data.size()));

                return xio::buffer(data,
                                   length > 0 ? static_cast<std::size_t>(length) : 0);
            }

            xio::const_buffer engine::put_input(
            ...
            const xio::error_code &engine::map_error_code(
                ...
                if (BIO_wpending(ext_bio_)) {
                    ec = xio::ssl::error::stream_truncated;
                    return ec;
                }
            ...
            engine::want engine::perform(int (engine::*op)(void *, std::size_t),
                ...
                if (ssl_error == SSL_ERROR_SYSCALL) {
                    if (sys_error == 0) {
                        ec = xio::ssl::error::unspecified_system_error;
                    } else {
                        ec = xio::error_code(sys_error,
                                             xio::error::get_ssl_category());
                    }
                ...
                if (ssl_error == SSL_ERROR_WANT_WRITE) {
                    ...
                    ec = xio::ssl::error::unexpected_result;
                    return want_nothing;
                }
            }
...
#if (OPENSSL_VERSION_NUMBER < 0x10000000L)
                xio::detail::static_mutex::scoped_lock lock(accept_mutex());
#endif // (OPENSSL_VERSION_NUMBER < 0x10000000L)
