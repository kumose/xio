...
#define XIO_BASIC_SOCKET_STREAMBUF_FWD_DECL
   ...
   * buffer.
   */
        const xio::error_code &error() const {
            return ec_;
        }
        ...
        int_type underflow() {
            ...
            for (;;) {
                ...
                detail::buffer_sequence_adapter<mutable_buffer, mutable_buffer>
                        bufs(xio::buffer(get_buffer_) + putback_max);
                ...
                // Operation failed.
                if (ec_ != xio::error::would_block
                    && ec_ != xio::error::try_again)
                    return traits_type::eof();
    ...
    private:
        ...
        void connect_to_endpoints(EndpointIterator begin, EndpointIterator end) {
            ...
            for (EndpointIterator i = begin; i != end; ++i) {
                ...
                // Operation failed.
                if (ec_ != xio::error::in_progress
                    && ec_ != xio::error::would_block)
                    continue;
