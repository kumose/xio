...
namespace nuraft {
    ...
    ptr<raft_server> raft_launcher::init(ptr<state_machine> sm,
        ...
        ptr<rpc_client_factory> rpc_cli_factory = xio_svc_;
        ...
        xio_listener_->listen(raft_instance_);
    ...
    bool raft_launcher::shutdown(size_t time_limit_sec) {
        ...
        if (xio_listener_) {
            xio_listener_->stop();
            xio_listener_->shutdown();
        }
        if (xio_svc_) {
            xio_svc_->stop();
            size_t count = 0;
            while (xio_svc_->get_active_workers() &&
        ...
        if (xio_svc_->get_active_workers()) return false;
        return true;
    }
}
