...
#include <xio/detail/composed_work.h>
...
using xio::detail::composed_work_guard;
using xio::detail::get_composed_io_executor;
using xio::detail::make_composed_io_executors;
using xio::detail::recycling_allocator;
using xio::detail::throw_error;
...
struct co_composed_state_default_cancellation_on_suspend_impl<Executors,
            Handler, Return, R(xio::error_code, Args...), Signatures...> {
    using promise_type = co_composed_promise<Executors, Handler, Return>;
...
public:
    ...
    template<typename... T>
    void operator()(const xio::error_code &ec, T &&... args) {
...
struct coroutine_traits<void, C &,
            xio::detail::co_composed_state<Executors, Handler, Return>, Args...> {
    using promise_type =
 ...
 *             xio::enable_terminal_cancellation());
 *
 ...
 *                 make_error_code(xio::error::operation_aborted));
 *
 *             auto [e2, n2] =
 *               co_await xio::async_write(socket,
 *                 xio::buffer(data, n1));
 *
 *             if (e2)
