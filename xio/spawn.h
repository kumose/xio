...
#include <xio/detail/exception.h>
...
namespace xio {
    ...
    namespace detail {
        ...
        protected:
            ...
            xio::cancellation_slot parent_cancellation_slot_;
            xio::cancellation_state cancellation_state_;
 ...
 */
   ...
   * } @endcode
   */
        basic_yield_context operator[](xio::error_code &ec) const {
 ...
 *
 * @code xio::spawn(my_strand, do_echo, xio::detached);
 *
 * // ...
 *
 * void do_echo(xio::yield_context yield)
 * {
 *   try
 *   {
 *     char data[128];
 ...
 *           xio::buffer(data), yield);
 *
 *       xio::async_write(my_socket,
 *           xio::buffer(data, length), yield);
 *     }
 *   }
