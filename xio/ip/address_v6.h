...
namespace xio {
    ...
    namespace ip {
        ...
        /**
 * The xio::ip::address_v6 class provides the ability to use and
 ...
 */
...
#else
            typedef xio::detail::array<unsigned char, 16> bytes_type;
#endif
  ...
  ASIO_DECL static address_v6 loopback() noexcept;
        ...
        private:
            ...
            xio::detail::in6_addr_type addr_;
...
namespace std {
    template<>
    struct hash<xio::ip::address_v6> {
        std::size_t operator()(const xio::ip::address_v6 &addr)
        const noexcept {
            const xio::ip::address_v6::bytes_type bytes = addr.to_bytes();
