...
namespace xio {
    ...
    namespace ip {
        ...
        /**
 * The xio::ip::basic_resolver_results class template is used to define
...
# if defined(ASIO_WINDOWS_RUNTIME)
            ...
            static basic_resolver_results create(
                ...
                const xio::detail::addrinfo_type &hints,
                const std::string &host_name, const std::string &service_name) {
                basic_resolver_results results;
                if (endpoints->Size) {
                    results.values_.reset(new values_type);
                        ...
                        results.values_->push_back(
                            basic_resolver_entry<InternetProtocol>(
                                typename InternetProtocol::endpoint(
                                    ip::make_address(
                                        xio::detail::winrt_utils::string(
                                            pair->RemoteHostName->CanonicalName)),
                                    xio::detail::winrt_utils::integer(
