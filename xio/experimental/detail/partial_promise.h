        ...
        namespace detail {
            ...
            struct partial_promise : partial_promise_base<Allocator> {
                auto initial_suspend() noexcept {
                    return xio::detail::suspend_always{};
                }
                ...
                auto final_suspend() noexcept {
                    struct awaitable_t {
                        ...
                        auto await_suspend(xio::detail::coroutine_handle<>) noexcept {
...
namespace std {
    ...
    struct coroutine_traits<
                xio::experimental::detail::partial_coro,
                Executor, Completion, Args...> {
        using promise_type =
        xio::experimental::detail::partial_promise<
            xio::associated_allocator_t<Completion> >;
    };
} // namespace std
    ...
    namespace experimental {
        namespace detail {
            ...
            partial_coro post_coroutine(Executor exec,
                ...
                post(exec, xio::append(std::move(token), std::move(args)...));
                co_return;
            }
            ...
            partial_coro post_coroutine(Context &ctx,
                                        CompletionToken token, Args &&... args) noexcept {
                post(ctx, xio::append(std::move(token), std::move(args)...));
                co_return;
            }
            ...
            partial_coro dispatch_coroutine(Executor exec,
                                            CompletionToken token, Args &&... args) noexcept {
                dispatch(exec, xio::append(std::move(token), std::move(args)...));
                co_return;
            }
            ...
            partial_coro dispatch_coroutine(Context &ctx,
                                            CompletionToken token, Args &&... args) noexcept {
                dispatch(ctx, xio::append(std::move(token), std::move(args)...));
                co_return;
            }
        } // namespace detail
    } // namespace experimental
