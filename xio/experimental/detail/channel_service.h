...
#include <xio/detail/completion_payload.h>
#include <xio/detail/completion_payload_handler.h>
#include <xio/detail/mutex.h>
#include <xio/detail/op_queue.h>
#include <xio/execution_context.h>
#include <xio/experimental/detail/channel_receive_op.h>
#include <xio/experimental/detail/channel_send_op.h>
#include <xio/experimental/detail/has_signature.h>
    ...
    namespace experimental {
        namespace detail {
            ...
            class channel_service
                    : public xio::detail::execution_context_service_base<
            ...
            public:
                ...
                struct base_implementation_type {
                    ...
                    xio::detail::op_queue<channel_operation> waiters_;
                ...
                // Constructor.
                channel_service(xio::execution_context &ctx);
            ...
            struct channel_service<Mutex>::implementation_type : base_implementation_type {
                ...
                >::value
                ,
                xio::detail::completion_payload<Signatures...>
                ,
                xio::detail::completion_payload<
                    Signatures...,
                    typename traits_type::receive_closed_signature
                >
                >
            ...
            struct channel_service<Mutex>::implementation_type<Traits, R()>
                ...
                >::value
                ,
                xio::detail::completion_payload<R()>
                ,
                xio::detail::completion_payload<
                    R(),
                    typename traits_type::receive_closed_signature
                ...
                // Get the element at the front of the buffer.
                payload_type buffer_front() {
                    return payload_type(xio::detail::completion_message < R() > (0));
                }
            ...
            struct channel_service<Mutex>::implementation_type<
                        Traits, R(xio::error_code)>
                    : channel_service::base_implementation_type {
                // The traits type associated with the channel.
                typedef typename Traits::template rebind<R(xio::error_code)>::other
                traits_type;
                ...
                >::value
                ,
                xio::detail::completion_payload<R(xio::error_code)>
                ,
                xio::detail::completion_payload<
                    R(xio::error_code),
                    typename traits_type::receive_closed_signature
                ...
                void buffer_push(payload_type payload) {
                    ...
                    if (last.count_ == 0) {
                        ...
                        xio::error_code value{last.value_};
            ...
            private:
                struct buffered_value {
                    xio::error_code value_;
                    std::size_t count_;
                };

                struct value_handler {
                    xio::error_code &target_;

                    template<typename... Args>
                    void operator()(const xio::error_code &value, Args &&...) {
                        target_ = value;
                    }
                };
