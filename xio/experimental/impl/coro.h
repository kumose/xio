...
#include <xio/experimental/detail/coro_completion_handler.h>
...
namespace xio {
    ...
    namespace experimental {
        ...
        namespace detail {
            ...
            template<>
            struct coro_error<xio::error_code> {
                static xio::error_code invalid() {
                    return xio::error::fault;
                }

                static xio::error_code cancelled() {
                    return xio::error::operation_aborted;
                }

                static xio::error_code interrupted() {
                    return xio::error::interrupted;
                }

                static xio::error_code done() {
                    return xio::error::broken_pipe;
                }
            };
                        ...
                        if constexpr (!coro_promise<Y, R, E, A>::is_noexcept) {
                            if ((hp.cancel->state.cancelled() != cancellation_type::none)
                                ...
                                xio::detail::throw_error(
                                    xio::error::operation_aborted, "coro-cancelled");
                            }
                        }
                        ...
                        if (hp.get_executor() == coro.get_executor()) {
                            ...
                            struct cancel_handler {
                                ...
                                void operator()(cancellation_type ct) {
                                    xio::dispatch(e, [ct, st = st]() mutable {
                                        auto &[sig, state] = *st;
                                        sig.emit(ct);
                                    });
                                }
...
# pragma warning(pop)
            ...
            struct coro_promise final :
                ...
                using cancellation_slot_type = xio::cancellation_slot;
                ...
                auto await_transform(this_coro::cancellation_state_t) const {
                    struct exec_helper {
                        const xio::cancellation_state &value;
                        ...
                        xio::cancellation_state await_resume() const noexcept {
                            return value;
                        }
                    };
                    assert(cancel);
        ...
        struct coro<Yield, Return, Executor, Allocator>::initiate_async_resume {
            ...
            typedef xio::cancellation_slot cancellation_slot_type;
