...
#include <xio/experimental/detail/coro_promise_allocator.h>
#include <xio/experimental/detail/partial_promise.h>
#include <xio/post.h>
 ...
 */
        ...
        struct coro {
            /// The traits of the coroutine. See xio::experimental::coro_traits
  /// for details.
   ...
   */
            ~coro() {
                if (coro_ != nullptr) {
                    ...
                    if (handle)
                        xio::dispatch(coro_->get_executor(), destroyer{handle});
                }
            }
   ...
   */
        ...
        template<typename T, typename Executor = xio::any_io_executor,
            typename Allocator = std::allocator<void> >
