...
namespace xio {
    ...
    namespace detail {
        ...
        struct default_connect_condition {
            ...
            bool operator()(const xio::error_code &, const Endpoint &) {
                return true;
            }
        };
    } // namespace detail
 ...
 * xio::connect(s, r.resolve(q)); @endcode
 */
 ...
 * empty, set to xio::error::not_found. Otherwise, contains the error
 ...
 */
    template<typename Protocol, typename Executor, typename EndpointSequence>
    typename Protocol::endpoint connect(basic_socket<Protocol, Executor> &s,
                                        const EndpointSequence &endpoints, xio::error_code &ec,
                                        constraint_t<
                                            is_endpoint_sequence<EndpointSequence>::value
                                        > = 0);
 ...
 * tcp::socket s(my_context);
 * xio::connect(s, e.begin(), e.end()); @endcode
 */
 ...
 * tcp::socket s(my_context);
 * xio::error_code ec;
 * xio::connect(s, e.begin(), e.end(), ec);
 * if (ec)
 * {
 *   // An error occurred.
 * } @endcode
 */
    template<typename Protocol, typename Executor, typename Iterator>
    Iterator connect(basic_socket<Protocol, Executor> &s,
                     Iterator begin, Iterator end, xio::error_code &ec);
 ...
 *   }
 * }; @endcode
 * It would be used with the xio::connect function as follows:
 * @code tcp::resolver r(my_context);
 * tcp::resolver::query q("host", "service");
 * tcp::socket s(my_context);
 * tcp::endpoint e = xio::connect(s,
 ...
 * tcp::resolver::results_type e = r.resolve(q);
 * tcp::socket s(my_context);
 * tcp::resolver::results_type::iterator i = xio::connect(
 ...
 */
    ...
    /**
 * @defgroup async_connect xio::async_connect
 *
 ...
 *   // xio::error::not_found. Otherwise, contains the
 ...
 * @code void(xio::error_code, typename Protocol::endpoint) @endcode
 *
 * @par Example
 * @code tcp::resolver r(my_context);
 * tcp::resolver::query q("host", "service");
 ...
 *   if (!ec)
 *   {
 *     xio::async_connect(s, results, connect_handler);
 *   }
 * }
 *
 * // ...
 ...
 *
 * @par Completion Signature
 * @code void(xio::error_code, Iterator) @endcode
 *
 * @par Example
