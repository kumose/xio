 ...
 *       {
 *         state_ = writing;
 *         xio::async_write(socket_, buffer_,
 *             xio::transfer_exactly(n),
 *             std::move(self));
 *       }
 *       break;
 *     case writing:
 ...
 *     xio::mutable_buffer buffer,
 *     CompletionToken&& token)
 *   -> decltype(
 *     xio::async_compose<CompletionToken,
 *       void(xio::error_code, std::size_t)>(
 *         std::declval<async_echo_implementation>(),
 *         token, socket))
 * {
 *   return xio::async_compose<CompletionToken,
 *     void(xio::error_code, std::size_t)>(
