...
#ifndef XIO_DETAIL_CONSUMING_BUFFERS_HPP
#define XIO_DETAIL_CONSUMING_BUFFERS_HPP
    ...
    namespace detail {
        ...
        public:
            ...
            prepared_buffers_type prepare(std::size_t max_size) {
                ...
                Buffer_Iterator next = xio::buffer_sequence_begin(buffers_);
                Buffer_Iterator end = xio::buffer_sequence_end(buffers_);

                std::advance(next, next_elem_);
                std::size_t elem_offset = next_elem_offset_;
                while (next != end && max_size > 0 && (result.count) < result.max_buffers) {
                    Buffer next_buf = Buffer(*next) + elem_offset;
                    result.elems[result.count] = xio::buffer(next_buf, max_size);
        ...
        public:
            ...
            Buffer prepare(std::size_t max_size) {
                return xio::buffer(buffer_ + total_consumed_, max_size);
            }
        ...
        public:
            ...
            std::array<Buffer, 2> prepare(std::size_t max_size) {
                ...
                result[0] = xio::buffer(result[0] + total_consumed_, max_size);
                result[1] = xio::buffer(
...
#endif // XIO_DETAIL_CONSUMING_BUFFERS_HPP
