...
namespace xio {
    ...
    namespace detail {
        ...
        win_iocp_io_context::win_iocp_io_context(
            xio::execution_context &ctx, bool own_thread)
            ...
            if (!iocp_.handle) {
                ...
                xio::detail::throw_error(ec, "iocp");
            }
        ...
        win_iocp_io_context::win_iocp_io_context(
            win_iocp_io_context::internal, xio::execution_context &ctx)
            : execution_context_service_base<win_iocp_io_context>(ctx),
              iocp_(),
              outstanding_work_(0),
              stopped_(0),
        ...
        xio::error_code win_iocp_io_context::register_handle(
        ...
        size_t win_iocp_io_context::run(xio::error_code &ec) {
        ...
        size_t win_iocp_io_context::run_one(xio::error_code &ec) {
            if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0) {
                stop();
                ec = xio::error_code();
                return 0;
        ...
        size_t win_iocp_io_context::wait_one(long usec, xio::error_code &ec) {
            if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0) {
                stop();
                ec = xio::error_code();
                return 0;
        ...
        size_t win_iocp_io_context::poll(xio::error_code &ec) {
            if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0) {
                stop();
                ec = xio::error_code();
                return 0;
        ...
        size_t win_iocp_io_context::poll_one(xio::error_code &ec) {
            if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0) {
                stop();
                ec = xio::error_code();
                return 0;
            ...
            if (::InterlockedExchange(&stopped_, 1) == 0) {
                if (::InterlockedExchange(&stop_event_posted_, 1) == 0) {
                    if (!::PostQueuedCompletionStatus(iocp_.handle, 0, 0, 0)) {
                        ...
                        xio::error_code ec(last_error,
                                           xio::error::get_system_category());
                        xio::detail::throw_error(ec, "pqcs");
                    }
                }
            }
        }
        ...
        void win_iocp_io_context::on_completion(win_iocp_operation *op,
                                                const xio::error_code &ec, DWORD bytes_transferred) {
            // Flag that the operation is ready for invocation.
            op->ready_ = 1;
        ...
        size_t win_iocp_io_context::do_one(DWORD msec,
            ...
            for (;;) {
                ...
                if (overlapped) {
                    ...
                    xio::error_code result_ec(last_error,
                                              xio::error::get_system_category());

                    // We may have been passed the last_error and bytes_transferred in the
                    // OVERLAPPED structure itself.
                    if (completion_key == overlapped_contains_result) {
                        result_ec = xio::error_code(static_cast<int>(op->Offset),
                                                    *reinterpret_cast<xio::error_category *>(op->Internal));
...
void win_iocp_io_context::do_add_timer_queue(timer_queue_base &queue) {
    ...
    if (!waitable_timer_.handle) {
        ...
        if (waitable_timer_.handle == 0) {
            ...
            xio::error_code ec(last_error,
                               xio::error::get_system_category());
            xio::detail::throw_error(ec, "timer");
        }
