...
xio::error_code win_iocp_socket_service_base::close(
    win_iocp_socket_service_base::base_implementation_type &impl,
    xio::error_code &ec) {
    if (is_open(impl)) {
...
xio::error_code win_iocp_socket_service_base::cancel(
    win_iocp_socket_service_base::base_implementation_type &impl,
    xio::error_code &ec) {
    if (!is_open(impl)) {
...
xio::error_code win_iocp_socket_service_base::do_open(
    win_iocp_socket_service_base::base_implementation_type &impl,
    int family, int type, int protocol, xio::error_code &ec) {
    if (is_open(impl)) {
    ...
    switch (type) {
        case SOCK_STREAM: impl.state_ = socket_ops::stream_oriented;
            break;
        case SOCK_DGRAM: impl.state_ = socket_ops::datagram_oriented;
            break;
        default: impl.state_ = 0;
            break;
    }
...
xio::error_code win_iocp_socket_service_base::do_assign(
    win_iocp_socket_service_base::base_implementation_type &impl,
    int type, socket_type native_socket, xio::error_code &ec) {
    if (is_open(impl)) {
        ec = xio::error::already_open;
        return ec;
    }
...
void win_iocp_socket_service_base::start_send_op(
    ...
    if (noop)
        ...
        iocp_service_.on_completion(op, xio::error::bad_descriptor);
    else {
...
void win_iocp_socket_service_base::start_accept_op(
    ...
    if (!is_open(impl))
        ...
        iocp_service_.on_completion(op, xio::error::already_open);
    else {
        xio::error_code ec;
...
void win_iocp_socket_service_base::restart_accept_op(
    ...
    if (cancel_requested)
        if (::InterlockedExchangeAdd(cancel_requested, 0) == 1)
            iocp_service_.on_completion(op, xio::error::operation_aborted);
...
int win_iocp_socket_service_base::start_connect_op(
    ...
    if ((impl.state_ & socket_ops::non_blocking) != 0
        ...
        if (socket_ops::connect(impl.socket_, addr, addrlen, op->ec_) != 0) {
            if (op->ec_ == xio::error::in_progress
                || op->ec_ == xio::error::would_block) {
