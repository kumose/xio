...
#ifndef XIO_DETAIL_SOCKET_OPS_IPP
#define XIO_DETAIL_SOCKET_OPS_IPP
    ...
    namespace detail {
        namespace socket_ops {
            ...
            inline void get_last_error(
                xio::error_code &ec, bool is_error_condition) {
                if (!is_error_condition) {
                    xio::error::clear(ec);
                } else {
#if defined(XIO_WINDOWS) || defined(XIO_CYGWIN_W32_SOCKETS)
                    ec = xio::error_code(WSAGetLastError(),
            ...
            socket_type sync_accept(socket_type s, state_type state,
                ...
                for (;;) {
                    ...
                    // Operation failed.
                    if (ec == xio::error::would_block
                        || ec == xio::error::try_again) {
                        if (state & user_set_non_blocking)
                            return invalid_socket;
                        // Fall through to retry operation.
                    } else if (ec == xio::error::connection_aborted) {
            ...
            void complete_iocp_accept(socket_type s, void *output_buffer,
                ...
                if (ec.value() == ERROR_NETNAME_DELETED)
                    ec = xio::error::connection_aborted;
...
# endif // defined(ENOTCAPABLE)
            ...
            int shutdown(socket_type s, int what, xio::error_code &ec) {
                if (s == invalid_socket) {
                    ec = xio::error::bad_descriptor;
                    return socket_error_retval;
                }
            ...
            int connect(socket_type s, const void *addr,
                ...
                if (result != 0 && ec == xio::error::try_again) {
                    if (static_cast<const socket_addr_type *>(addr)->sa_family == AF_UNIX)
                        ec = xio::error::in_progress;
                    else
                        ec = xio::error::no_buffer_space;
                }
            ...
            void sync_connect(socket_type s, const void *addr,
                ...
                ec = xio::error_code(connect_error,
                                     xio::error::get_system_category());
            }

#if defined(XIO_HAS_IOCP)

            void complete_iocp_connect(socket_type s, xio::error_code &ec) {
                // Map non-portable errors to their portable counterparts.
                switch (ec.value()) {
                    ...
                    case ERROR_NETWORK_UNREACHABLE:
                        ec = xio::error::network_unreachable;
                        break;
                    case ERROR_HOST_UNREACHABLE:
                        ec = xio::error::host_unreachable;
                        break;
                    case ERROR_SEM_TIMEOUT:
                        ec = xio::error::timed_out;
                        break;
                    default:
                        break;
                }
            ...
            bool non_blocking_connect(socket_type s, xio::error_code &ec) {
  ...
  || defined(__SYMBIAN32__)
            ...
            bool sockatmark(socket_type s, xio::error_code &ec) {
                if (s == invalid_socket) {
                    ec = xio::error::bad_descriptor;
                    return false;
                }
...
# endif // defined(ENOTTY)
            ...
            size_t available(socket_type s, xio::error_code &ec) {
                if (s == invalid_socket) {
                    ec = xio::error::bad_descriptor;
                    return 0;
                }
            ...
            int listen(socket_type s, int backlog, xio::error_code &ec) {
                if (s == invalid_socket) {
                    ec = xio::error::bad_descriptor;
                    return socket_error_retval;
                }
            ...
            size_t sync_recv1(socket_type s, state_type state, void *data,
                              size_t size, int flags, xio::error_code &ec) {
                if (s == invalid_socket) {
                    ec = xio::error::bad_descriptor;
                    return 0;
                }
            ...
            signed_size_type recvfrom(socket_type s, buf *bufs, size_t count,
                                      int flags, void *addr, std::size_t *addrlen, xio::error_code &ec) {
#if defined(XIO_WINDOWS) || defined(XIO_CYGWIN_W32_SOCKETS)
                // Receive some data.
                DWORD recv_buf_count = static_cast<DWORD>(count);
                DWORD bytes_transferred = 0;
            ...
            inline signed_size_type call_recvfrom(SockLenType msghdr::*, socket_type s,
                ...
                signed_size_type result = ::recvfrom(s, static_cast<char *>(data), size,
                                                     flags, static_cast<socket_addr_type *>(addr),
                                                     addrlen ? &tmp_addrlen : 0);
                if (addrlen)
                    *addrlen = (std::size_t) tmp_addrlen;
                return result;
            ...
            signed_size_type recvmsg(socket_type s, buf *bufs, size_t count,
                                     int in_flags, int &out_flags, xio::error_code &ec) {
#if defined(XIO_WINDOWS) || defined(XIO_CYGWIN_W32_SOCKETS)
                out_flags = 0;
            ...
            bool non_blocking_recvmsg(socket_type s,
                                      buf *bufs, size_t count, int in_flags, int &out_flags,
                                      xio::error_code &ec, size_t &bytes_transferred) {
                in_flags |= XIO_OS_DEF(MSG_DONTWAIT);
                for (;;) {
                    // Read some data.
                    signed_size_type bytes = socket_ops::recvmsg(
                        s, bufs, count, in_flags, out_flags, ec);
            ...
            int setsockopt(socket_type s, state_type &state, int level, int optname,
                           const void *optval, std::size_t optlen, xio::error_code &ec) {
                if (s == invalid_socket) {
                    ec = xio::error::bad_descriptor;
                    return socket_error_retval;
                }
  ...
  || defined(__OpenBSD__) || defined(__QNX__)
            ...
            int getsockopt(socket_type s, state_type state, int level, int optname,
                           void *optval, size_t *optlen, xio::error_code &ec) {
                if (s == invalid_socket) {
                    ec = xio::error::bad_descriptor;
                    return socket_error_retval;
                }
            ...
            int getpeername(socket_type s, void *addr, std::size_t *addrlen,
                            bool cached, xio::error_code &ec) {
                if (s == invalid_socket) {
                    ec = xio::error::bad_descriptor;
                    return socket_error_retval;
                }

#if defined(XIO_WINDOWS) && !defined(XIO_WINDOWS_APP) \
  || defined(XIO_CYGWIN_W32_SOCKETS)
                if (cached) {
...
#else // defined(XIO_WINDOWS) && !defined(XIO_WINDOWS_APP)
                // || defined(XIO_CYGWIN_W32_SOCKETS)
                (void) cached;
#endif // defined(XIO_WINDOWS) && !defined(XIO_WINDOWS_APP)
                // || defined(XIO_CYGWIN_W32_SOCKETS)
...
#elif defined(XIO_CYGWIN_W32_SOCKETS)
                TIMEVAL tv;
  ...
  || defined(__SYMBIAN32__)
            ...
            int poll_connect(socket_type s, int msec, xio::error_code &ec) {
                if (s == invalid_socket) {
                    ec = xio::error::bad_descriptor;
                    return socket_error_retval;
                }
  ...
  || defined(__SYMBIAN32__)
            ...
            const char *inet_ntop(int af, const void *src, char *dest, size_t length,
                ...
                if (af == XIO_OS_DEF(AF_INET)) {
                    sprintf_s(dest, length, "%u.%u.%u.%u",
                              bytes[0], bytes[1], bytes[2], bytes[3]);
                    return dest;
                } else if (af == XIO_OS_DEF(AF_INET6)) {
                    size_t n = 0, b = 0, z = 0;
                    while (n < length && b < 16) {
                ...
                if (af != XIO_OS_DEF(AF_INET) && af != XIO_OS_DEF(AF_INET6)) {
                    ec = xio::error::address_family_not_supported;
                    return 0;
                }
                ...
                if (af == XIO_OS_DEF(AF_INET)) {
                    ...
                    address.v4.sin_family = XIO_OS_DEF(AF_INET);
                    address.v4.sin_port = 0;
                    memcpy(&address.v4.sin_addr, src, sizeof(in4_addr_type));
                } else // AF_INET6
                {
                    address_length = sizeof(sockaddr_in6_type);
                    address.v6.sin6_family = XIO_OS_DEF(AF_INET6);
                ...
                if (result == 0 && !ec)
                    ec = xio::error::invalid_argument;
                if (result != 0 && af == XIO_OS_DEF(AF_INET6) && scope_id != 0) {
            ...
            int inet_pton(int af, const char *src, void *dest,
                ...
                const bool is_v6 = (af == XIO_OS_DEF(AF_INET6));
            ...
            int gethostname(char *name, int namelen, xio::error_code &ec) {
#if defined(XIO_WINDOWS_RUNTIME)
                try {
            ...
            inline xio::error_code translate_netdb_error(int error) {
                switch (error) {
                    case 0:
                        return xio::error_code();
                    case HOST_NOT_FOUND:
                        return xio::error::host_not_found;
                    case TRY_AGAIN:
                        return xio::error::host_not_found_try_again;
                    case NO_RECOVERY:
                        return xio::error::no_recovery;
                    case NO_DATA:
            ...
            inline hostent *gethostbyaddr(const char *addr, int length, int af,
                                          hostent *result, char *buffer, int buflength, xio::error_code &ec) {
#if defined(XIO_WINDOWS) || defined(XIO_CYGWIN_W32_SOCKETS)
                (void) (buffer);
                (void) (buflength);
            ...
            inline hostent *gethostbyname(const char *name, int af, struct hostent *result,
                                          char *buffer, int buflength, int ai_flags, xio::error_code &ec) {
#if defined(XIO_WINDOWS) || defined(XIO_CYGWIN_W32_SOCKETS)
                (void) (buffer);
                (void) (buflength);
                (void) (ai_flags);
                    ...
                    if (hints->ai_flags & AI_PASSIVE) {
                        ...
                        switch (hints->ai_family) {
                            case XIO_OS_DEF(AF_INET):
                                ...
                                search[search_count].family = XIO_OS_DEF(AF_INET);
                                ++search_count;
                                break;
                            case XIO_OS_DEF(AF_INET6):
                                search[search_count].host = "0::0";
                                search[search_count].family = XIO_OS_DEF(AF_INET6);
                                ++search_count;
                                break;
                            case XIO_OS_DEF(AF_UNSPEC):
                                search[search_count].host = "0::0";
                                search[search_count].family = XIO_OS_DEF(AF_INET6);
                                ++search_count;
                                search[search_count].host = "0.0.0.0";
                ...
                *next = &ai->ai_next;
                ...
                switch (ai->ai_family) {
                    case XIO_OS_DEF(AF_INET): {
                        ...
                        if (sinptr == 0)
                            return EAI_MEMORY;
                        sinptr->sin_family = XIO_OS_DEF(AF_INET);
                    ...
                    case XIO_OS_DEF(AF_INET6): {
                        ...
                        if (sin6ptr == 0)
                            return EAI_MEMORY;
                        sin6ptr->sin6_family = XIO_OS_DEF(AF_INET6);
            ...
            inline int getaddrinfo_emulation(const char *host, const char *service,
                ...
                hints.ai_family = XIO_OS_DEF(AF_UNSPEC);
                if (hintsp)
                    hints = *hintsp;
                ...
                for (gai_search *sptr = search; sptr < search + search_count; ++sptr) {
                    ...
                    xio::error_code ec;
                    if (socket_ops::inet_pton(XIO_OS_DEF(AF_INET),
                                              sptr->host, &inaddr, 0, ec) == 1) {
                        if (hints.ai_family != XIO_OS_DEF(AF_UNSPEC)
                            && hints.ai_family != XIO_OS_DEF(AF_INET)) {
                            freeaddrinfo_emulation(aihead);
                            gai_free(canon);
                            return EAI_FAMILY;
                        }
                        if (sptr->family == XIO_OS_DEF(AF_INET)) {
                            rc = gai_aistruct(&ainext, &hints, &inaddr, XIO_OS_DEF(AF_INET));
                            if (rc != 0) {
                                freeaddrinfo_emulation(aihead);
                                gai_free(canon);
                                return rc;
                    ...
                    if (socket_ops::inet_pton(XIO_OS_DEF(AF_INET6),
                                              sptr->host, &in6addr, 0, ec) == 1) {
                        if (hints.ai_family != XIO_OS_DEF(AF_UNSPEC)
                            && hints.ai_family != XIO_OS_DEF(AF_INET6)) {
                            freeaddrinfo_emulation(aihead);
                            gai_free(canon);
                            return EAI_FAMILY;
                        }
                        if (sptr->family == XIO_OS_DEF(AF_INET6)) {
                    ...
                    hostent *hptr = socket_ops::gethostbyname(sptr->host,
                                                              sptr->family, &hent, hbuf, sizeof(hbuf), hints.ai_flags,
                                                              ec);
                    if (hptr == 0) {
                        if (search_count == 2) {
                        ...
                        freeaddrinfo_emulation(aihead);
                        gai_free(canon);
                        if (ec == xio::error::host_not_found)
                            return EAI_NONAME;
                        if (ec == xio::error::host_not_found_try_again)
                            return EAI_AGAIN;
                        if (ec == xio::error::no_recovery)
                            return EAI_FAIL;
                        if (ec == xio::error::no_data)
                            return EAI_NONAME;
                        return EAI_NONAME;
                    }
            ...
            inline xio::error_code getnameinfo_emulation(
                ...
                switch (sa->sa_family) {
                    case XIO_OS_DEF(AF_INET):
                        if (salen != sizeof(sockaddr_in4_type)) {
                            return ec = xio::error::invalid_argument;
                        }
                    ...
                    case XIO_OS_DEF(AF_INET6):
                        ...
                        break;
                    default:
                        return ec = xio::error::address_family_not_supported;
                }
                    ...
                    if (flags & NI_NUMERICHOST) {
                        ...
                        if (hptr && hptr->h_name && hptr->h_name[0] != '\0') {
                            ...
                            if (flags & NI_NAMEREQD) {
                                return ec = xio::error::host_not_found;
                            }
                ...
                if (serv && servlen > 0) {
                    if (flags & NI_NUMERICSERV) {
                        if (servlen < 6) {
                            return ec = xio::error::no_buffer_space;
                        }
#if defined(XIO_HAS_SNPRINTF)
            ...
            inline xio::error_code translate_addrinfo_error(int error) {
                switch (error) {
                    case 0:
                        return xio::error_code();
                    case EAI_AGAIN:
                    ...
                    case EAI_FAIL:
                        return xio::error::no_recovery;
                    case EAI_FAMILY:
                        return xio::error::address_family_not_supported;
                    case EAI_MEMORY:
                        return xio::error::no_memory;
                    case EAI_NONAME:
                    ...
                    case EAI_NODATA:
                        ...
                        return xio::error::host_not_found;
                    case EAI_SERVICE:
                        return xio::error::service_not_found;
                    case EAI_SOCKTYPE:
                        return xio::error::socket_type_not_supported;
                    default: // Possibly the non-portable EAI_SYSTEM.
#if defined(XIO_WINDOWS) || defined(XIO_CYGWIN_W32_SOCKETS)
                        return xio::error_code(
                            WSAGetLastError(), xio::error::get_system_category());
#else
                        return xio::error_code(
                            errno, xio::error::get_system_category());
#endif
                }
            }

            xio::error_code getaddrinfo(const char *host,
...
# else
                ...
                return ec = translate_addrinfo_error(error);
# endif
#elif !defined(XIO_HAS_GETADDRINFO)
                int error = getaddrinfo_emulation(host, service, &hints, result);
                return ec = translate_addrinfo_error(error);
#else
                int error = ::getaddrinfo(host, service, &hints, result);
            ...
            xio::error_code background_getaddrinfo(
...
# endif
            ...
            xio::error_code getnameinfo(const void *addr,
                                        std::size_t addrlen, char *host, std::size_t hostlen,
                                        char *serv, std::size_t servlen, int flags, xio::error_code &ec) {
#if defined(XIO_WINDOWS) || defined(XIO_CYGWIN_W32_SOCKETS)
# if defined(XIO_HAS_GETADDRINFO)
                // Building for Windows XP, Windows Server 2003, or later.
                clear_last_error();
...
# endif
            ...
            xio::error_code sync_getnameinfo(const void *addr,
            ...
            xio::error_code background_getnameinfo(
                const weak_cancel_token_type &cancel_token,
                const void *addr, std::size_t addrlen,
...
#endif // XIO_DETAIL_SOCKET_OPS_IPP
