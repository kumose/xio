...
namespace xio {
    ...
    namespace detail {
        ...
        void ASIO_VERSIONED_NAME(signal_handler)(
        int signal_number
        )
{
...
# endif // defined(ASIO_HAS_IO_URING_AS_DEFAULT)
                ...
                case execution_context::fork_child:
                    if (state->fork_prepared_) {
                        xio::detail::signal_blocker blocker;
                        close_descriptors();
                        open_descriptors();
                        int read_descriptor = state->read_descriptor_;
                        state->fork_prepared_ = false;
...
# endif // defined(ASIO_HAS_IO_URING_AS_DEFAULT)
        ...
        xio::error_code signal_set_service::add(
            signal_set_service::implementation_type &impl, int signal_number,
            signal_set_base::flags_t f, xio::error_code &ec) {
...
# if defined(ASIO_HAS_SIGACTION)
                ...
                sa.sa_handler = ASIO_VERSIONED_NAME(signal_handler);
                sigfillset(&sa.sa_mask);
                if (f != signal_set_base::flags::dont_care)
                    sa.sa_flags = static_cast<int>(f);
                if (::sigaction(signal_number, &sa, 0) == -1)
# else // defined(ASIO_HAS_SIGACTION)
                if (::signal(signal_number, ASIO_VERSIONED_NAME(signal_handler))
                    == SIG_ERR)
# endif // defined(ASIO_HAS_SIGACTION)
                {
...
# endif // defined(ASIO_HAS_SIGACTION)
        ...
        xio::error_code signal_set_service::remove(
            signal_set_service::implementation_type &impl,
...
# endif // defined(ASIO_HAS_SIGACTION)
        ...
        xio::error_code signal_set_service::clear(
            signal_set_service::implementation_type &impl,
            xio::error_code &ec) {
            signal_state *state = get_signal_state();
            static_mutex::scoped_lock lock(state->mutex_);
