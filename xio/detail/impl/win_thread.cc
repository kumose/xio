...
#include <xio/detail/win_thread.h>
#include <xio/error.h>
    ...
    namespace detail {
        ...
        win_thread::func_base *win_thread::start_thread(
            ...
            if (!arg->entry_event_) {
                ...
                xio::detail::throw_error(ec, "thread.entry_event");
            }
            ...
            if (!arg->exit_event_) {
                ...
                xio::error_code ec(last_error,
                                   xio::error::get_system_category());
                xio::detail::throw_error(ec, "thread.exit_event");
            }

            unsigned int thread_id = 0;
            arg->thread_ = reinterpret_cast<HANDLE>(::_beginthreadex(0,
                                                                     stack_size, win_thread_function, arg, 0,
                                                                     &thread_id));
            if (!arg->thread_) {
                DWORD last_error = ::GetLastError();
                ::CloseHandle(arg->entry_event_);
