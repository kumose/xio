...
#ifndef XIO_DETAIL_IMPL_DEV_POLL_REACTOR_IPP
#define XIO_DETAIL_IMPL_DEV_POLL_REACTOR_IPP
...
namespace xio {
    ...
    namespace detail {
        dev_poll_reactor::dev_poll_reactor(xio::execution_context &ctx)
            : xio::detail::execution_context_service_base<dev_poll_reactor>(ctx),
            ...
            if (fork_ev == xio::execution_context::fork_child) {
                ...
                for (int i = 0; i < max_ops; ++i) {
                    ...
                    for (; iter != end; ++iter) {
                        ...
                        switch (i) {
                            case read_op: pending_ev.events |= POLLIN;
                                break;
                            case write_op: pending_ev.events |= POLLOUT;
                                break;
                            case except_op: pending_ev.events |= POLLPRI;
                                break;
                            default: break;
                        }
                    }
        ...
        int dev_poll_reactor::register_internal_descriptor(int op_type,
            ...
            switch (op_type) {
                case read_op: ev.events |= POLLIN;
                    break;
                case write_op: ev.events |= POLLOUT;
                    break;
                case except_op: ev.events |= POLLPRI;
                    break;
                default: break;
            }
            interrupter_.interrupt();
        ...
        void dev_poll_reactor::cancel_ops(socket_type descriptor,
                                          dev_poll_reactor::per_descriptor_data &) {
            xio::detail::mutex::scoped_lock lock(mutex_);
            cancel_ops_unlocked(descriptor, xio::error::operation_aborted);
        }
        ...
        void dev_poll_reactor::cancel_ops_by_key(socket_type descriptor,
            ...
            bool need_interrupt = op_queue_[op_type].cancel_operations_by_key(
                descriptor, ops, cancellation_key, xio::error::operation_aborted);
        ...
        void dev_poll_reactor::run(long usec, op_queue<operation> &ops) {
            ...
            if (events_size > 0) {
                ...
                if (result != static_cast<int>(events_size)) {
                    xio::error_code ec = xio::error_code(
        ...
        int dev_poll_reactor::do_dev_poll_create() {
            ...
            if (fd == -1) {
                xio::error_code ec(errno,
                                   xio::error::get_system_category());
                xio::detail::throw_error(ec, "/dev/poll");
            }
            return fd;
        }
...
#endif // XIO_DETAIL_IMPL_DEV_POLL_REACTOR_IPP
