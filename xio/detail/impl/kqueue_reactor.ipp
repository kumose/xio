...
namespace xio {
    ...
    namespace detail {
        kqueue_reactor::kqueue_reactor(xio::execution_context &ctx)
            ...
            if (fork_ev == xio::execution_context::fork_child) {
                ...
                if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1) {
                    xio::error_code ec(errno,
                                        xio::error::get_system_category());
                    xio::detail::throw_error(ec, "kqueue interrupter registration");
                }
                ...
                for (descriptor_state *state = registered_descriptors_.first();
                    ...
                    if (state->num_kevents_ > 0) {
                        ...
                        if (::kevent(kqueue_fd_, events, state->num_kevents_, 0, 0, 0) == -1) {
                            xio::error_code ec(errno,
                                                xio::error::get_system_category());
                            xio::detail::throw_error(ec, "kqueue re-registration");
                        }
                    }
                }
            }
...
int kqueue_reactor::do_kqueue_create() {
    ...
    if (fd == -1) {
        xio::error_code ec(errno,
                            xio::error::get_system_category());
        xio::detail::throw_error(ec, "kqueue");
    }
    return fd;
}
