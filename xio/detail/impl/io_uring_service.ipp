...
namespace xio {
    ...
    namespace detail {
        ...
        io_uring_service::io_uring_service(xio::execution_context &ctx)
        ...
        void io_uring_service::notify_fork(
            ...
            switch (fork_ev) {
                case xio::execution_context::fork_prepare: {
...
register_with_reactor();
  ...
  case xio::execution_context::fork_parent :
    // Restart the timeout and eventfd operations.
    update_timeout();
register_with_reactor();
    break;

  case xio::execution_context::fork_child : {
...
post_submit_sqes_op(lock, 0);
  ...
  {
    xio::error_code ec(ENOBUFS,
        xio::error::get_system_category());
    xio::detail::throw_error(ec, "io_uring_get_sqe");
  }
}
    ...
    for (std::size_t i = 0; i < rings_.size(); ++i) {
        ...
        if (result < 0) {
            xio::error_code ec(-result,
                                xio::error::get_system_category());
            xio::detail::throw_error(ec, "io_uring_register_buffers");
        }
    }
}
    ...
    while (io_uring_operation *op = io_obj->queues_[op_type].op_queue_.front()) {
        ...
        if (op->cancellation_key_ == cancellation_key) {
            if (first) {
                ...
                op->ec_ = xio::error::operation_aborted;
                ops.push(op);
            }
        } else
            other_ops.push(op);
...
void io_uring_service::init_ring() {
    const unsigned int entries = ring_size_ != 0
                                     ? ring_size_
                                     : static_cast<unsigned int>(default_ring_size);
    for (std::size_t i = 0; i < rings_.size(); ++i) {
...
event_fd_= ::eventfd (0, EFD_CLOEXEC| EFD_NONBLOCK);
  if (event_fd_<0) {
    ...
    for (std::size_t i = 0; i < rings_.size(); ++i)
        ::io_uring_queue_exit(&rings_[i].ring_);
    xio::detail::throw_error(ec, "eventfd");
}
...
operation *io_uring_service::io_queue::perform_io(int result) {
    ...
    if (result != -ECANCELED || cancel_requested_) {
        if (io_uring_operation *op = op_queue_.front()) {
            if (result < 0) {
                op->ec_.assign(-result, xio::error::get_system_category());
...
service->post_submit_sqes_op(lock, ring_index);
    ...
    else
    {
      ...
      while (io_uring_operation* op = op_queue_.front())
      {
        op->ec_ = xio::error::no_buffer_space;
        op_queue_.pop();
        io_cleanup.ops_.push(op);
      }
    }
