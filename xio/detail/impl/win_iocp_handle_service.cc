...
namespace xio {
    ...
    namespace detail {
        class win_iocp_handle_service::overlapped_wrapper
                : public OVERLAPPED {
        public:
            explicit overlapped_wrapper(xio::error_code &ec) {
                Internal = 0;
                InternalHigh = 0;
                Offset = 0;
                OffsetHigh = 0;
        ...
        win_iocp_handle_service::win_iocp_handle_service(execution_context &context)
            : execution_context_service_base<win_iocp_handle_service>(context),
              iocp_service_(xio::use_service<win_iocp_io_context>(context)),
              nt_set_info_(0),
              mutex_(),
              impl_list_(0) {
        }
        ...
        xio::error_code win_iocp_handle_service::assign(
            win_iocp_handle_service::implementation_type &impl,
        ...
        xio::error_code win_iocp_handle_service::close(
            win_iocp_handle_service::implementation_type &impl,
            xio::error_code &ec) {
            if (is_open(impl)) {
        ...
        xio::error_code win_iocp_handle_service::cancel(
            win_iocp_handle_service::implementation_type &impl,
            xio::error_code &ec) {
            if (!is_open(impl)) {
        ...
        void win_iocp_handle_service::start_write_op(
            win_iocp_handle_service::implementation_type &impl, uint64_t offset,
            const xio::const_buffer &buffer, operation *op) {
        ...
        void win_iocp_handle_service::start_read_op(
            win_iocp_handle_service::implementation_type &impl, uint64_t offset,
            const xio::mutable_buffer &buffer, operation *op) {
            update_cancellation_thread_id(impl);
            iocp_service_.work_started();
