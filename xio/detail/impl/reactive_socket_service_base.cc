    ...
    namespace detail {
        reactive_socket_service_base::reactive_socket_service_base(
            ...
            : reactor_(use_service<reactor>(context)),
              extra_state_(
                  xio::config(context).get(
                      "reactor", "reset_edge_on_partial_read", 0)
                      ? socket_ops::reset_edge_on_partial_read
                      : 0) {
            reactor_.init_task();
        }
        ...
        xio::error_code reactive_socket_service_base::close(
            reactive_socket_service_base::base_implementation_type &impl,
            xio::error_code &ec) {
            if (is_open(impl)) {
                XIO_HANDLER_OPERATION((reactor_.context(),
        ...
        xio::error_code reactive_socket_service_base::cancel(
            reactive_socket_service_base::base_implementation_type &impl,
            xio::error_code &ec) {
            if (!is_open(impl)) {
                ec = xio::error::bad_descriptor;
        ...
        xio::error_code reactive_socket_service_base::do_open(
            reactive_socket_service_base::base_implementation_type &impl,
        ...
        xio::error_code reactive_socket_service_base::do_assign(
