    ...
    namespace detail {
        ...
        public:
            ...
            void sync(Windows::Foundation::IAsyncAction^ action,
                ...
                auto promise = std::make_shared<std::promise<xio::error_code> >();
                auto future = promise->get_future();

                action->Completed = ref
                new AsyncActionCompletedHandler(
                    [promise](IAsyncAction^ action, AsyncStatus status) {
                        switch (status) {
                            case AsyncStatus::Canceled:
                                promise->set_value(xio::error::operation_aborted);
                                break;
            ...
            void async(Windows::Foundation::IAsyncAction^ action,
                ...
                auto on_completed = ref
                new AsyncActionCompletedHandler(
                    [this, handler](IAsyncAction^ action, AsyncStatus status) {
                        switch (status) {
                            case AsyncStatus::Canceled:
                                handler->ec_ = xio::error::operation_aborted;
                                break;
                            case AsyncStatus::Started:
                                return;
                            case AsyncStatus::Completed:
