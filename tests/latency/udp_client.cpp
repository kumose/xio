...
int main(int argc, char* argv[])
{
  ...
  udp::endpoint target(xio::ip::make_address(ip), first_port);
  ...
  for (int i = 0; i < num_samples; ++i)
  {
    ...
    xio::error_code ec;
    socket.send_to(xio::buffer(write_buf), target, 0, ec);

    do socket.receive(xio::buffer(read_buf), 0, ec);
    while (ec == xio::error::would_block);
