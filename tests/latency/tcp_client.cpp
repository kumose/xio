...
struct transfer_all
{
  ...
  std::size_t operator()(const xio::error_code& ec, std::size_t)
  {
    return (ec && ec != xio::error::would_block) ? 0 : ~0;
  }
};
...
int main(int argc, char* argv[])
{
  ...
  for (int i = 0; i < num_connections; ++i)
  {
    ...
    tcp::endpoint target(xio::ip::make_address(ip), port);
    s->connect(target);
