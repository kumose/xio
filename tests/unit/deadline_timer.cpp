...
#include <xio/deadline_timer.h>

#include "unit_test.hpp"

#if !defined(XIO_NO_DEPRECATED) \
  && defined(XIO_HAS_BOOST_DATE_TIME)
...
void decrement_to_zero(xio::deadline_timer* t, int* count)
{
  if (*count > 0)
  {
    --(*count);
...
void cancel_timer(xio::deadline_timer* t)
{
  std::size_t num_cancelled = t->cancel();
  XIO_CHECK(num_cancelled == 1);
}

void cancel_one_timer(xio::deadline_timer* t)
{
...
void deadline_timer_test()
{
  ...
  xio::deadline_timer t1(ioc, seconds(1));
  t1.wait();
  ...
  xio::deadline_timer t2(ioc, seconds(1) + microseconds(500000));
  t2.wait();
  ...
  xio::deadline_timer t3(ioc, seconds(5));
  ...
  xio::deadline_timer t4(ioc, seconds(1));
  ...
  // counter.
  xio::deadline_timer t7(ioc, seconds(3));
  t7.async_wait(boost::bind(increment_if_not_cancelled, &count,
        xio::placeholders::error));
  t7.async_wait(boost::bind(increment_if_not_cancelled, &count,
        xio::placeholders::error));
  xio::deadline_timer t8(ioc, seconds(1));
...
void timer_handler(const xio::error_code&)
{
}
...
void deadline_timer_thread_test()
{
  ...
  xio::deadline_timer t1(ioc);
  xio::deadline_timer t2(ioc);
  int count = 0;

  xio::thread th(boost::bind(io_context_run, &ioc));
...
xio::deadline_timer make_timer(xio::io_context& ioc, int* count)
{
  xio::deadline_timer t(ioc);
...
void deadline_timer_move_test()
{
  ...
  xio::deadline_timer t1 = make_timer(io_context1, &count);
  xio::deadline_timer t2 = make_timer(io_context2, &count);
  xio::deadline_timer t3 = std::move(t1);
...
(
  "deadline_timer",
  XIO_TEST_CASE(deadline_timer_test)
  XIO_TEST_CASE(deadline_timer_cancel_test)
  XIO_TEST_CASE(deadline_timer_custom_allocation_test)
  XIO_TEST_CASE(deadline_timer_thread_test)
  XIO_TEST_CASE(deadline_timer_async_result_test)
  XIO_TEST_CASE(deadline_timer_move_test)
)
