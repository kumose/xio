...
void test_query()
{
  exec::blocking_t result1 = xio::query(Executor(), Param());
  XIO_CHECK(result1 == ExpectedResult());

  Executor ex1 = {};
  exec::blocking_t result2 = xio::query(ex1, Param());
  XIO_CHECK(result2 == ExpectedResult());

  const Executor ex2 = {};
  exec::blocking_t result3 = xio::query(ex2, Param());
...
void test_constexpr_query()
{
  ...
  constexpr exec::blocking_t result1 = xio::query(ex1, Param());
  XIO_CHECK(result1 == ExpectedResult());
}
...
void test_vars()
{
  XIO_CHECK(s() == exec::blocking);
  XIO_CHECK(s() != exec::blocking.possibly);
  XIO_CHECK(s() != exec::blocking.always);
  XIO_CHECK(s() != exec::blocking.never);
  XIO_CHECK(n1() == exec::blocking.possibly);
  XIO_CHECK(n1() != exec::blocking.always);
  XIO_CHECK(n1() != exec::blocking.never);
  XIO_CHECK(n2() == exec::blocking.always);
  XIO_CHECK(n2() != exec::blocking.possibly);
  XIO_CHECK(n2() != exec::blocking.never);
  XIO_CHECK(n3() == exec::blocking.never);
  XIO_CHECK(n3() != exec::blocking.possibly);
  XIO_CHECK(n3() != exec::blocking.always);
}
