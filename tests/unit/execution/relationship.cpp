...
void test_query()
{
  exec::relationship_t result1 = xio::query(Executor(), Param());
  XIO_CHECK(result1 == ExpectedResult());

  Executor ex1 = {};
  exec::relationship_t result2 = xio::query(ex1, Param());
  XIO_CHECK(result2 == ExpectedResult());

  const Executor ex2 = {};
  exec::relationship_t result3 = xio::query(ex2, Param());
...
void test_constexpr_query()
{
  ...
  constexpr exec::relationship_t result1 = xio::query(ex1, Param());
  XIO_CHECK(result1 == ExpectedResult());
}
...
void test_vars()
{
  XIO_CHECK(s() == exec::relationship);
  XIO_CHECK(s() != exec::relationship.fork);
  XIO_CHECK(s() != exec::relationship.continuation);
  XIO_CHECK(n1() == exec::relationship.fork);
  XIO_CHECK(n1() != exec::relationship.continuation);
  XIO_CHECK(n2() == exec::relationship.continuation);
  XIO_CHECK(n2() != exec::relationship.fork);
}
