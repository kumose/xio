...
void test()
{
  ...
  try
  {
    ...
    xio::uint64_t s1 = file1.size();
    (void)s1;
    xio::uint64_t s2 = file1.size(ec);
    (void)s2;

    file1.resize(xio::uint64_t(0));
    file1.resize(xio::uint64_t(0), ec);
    ...
    xio::uint64_t s3 = file1.seek(0, stream_file::seek_set);
    (void)s3;
    xio::uint64_t s4 = file1.seek(0, stream_file::seek_set, ec);
    (void)s4;
...
(
  "stream_file",
  XIO_COMPILE_TEST_CASE(stream_file_compile::test)
)
