...
{
public:
  explicit impl_0_completion_args(xio::io_context& ioc)
    : ioc_(ioc),
      state_(starting)
  {
  }
  ...
  void operator()(Self& self)
  {
    ...
    case starting:
      state_ = posting;
      xio::post(ioc_, static_cast<Self&&>(self));
      break;
    case posting:
      self.complete();
      break;
    ...
    CompletionToken&& token)
{
  return xio::async_initiate<CompletionToken, void()>(
      xio::composed(impl_0_completion_args(ioc)), token);
}
...
{
public:
  explicit impl_1_completion_arg(xio::io_context& ioc)
    : ioc_(ioc),
      state_(starting)
  {
  }
    ...
    CompletionToken&& token)
{
  return xio::async_initiate<CompletionToken, void(int)>(
      xio::composed(impl_1_completion_arg(ioc)), token);
}
...
typedef xio::enable_terminal_cancellation default_filter;
...
public:
  ...
  template <typename Self>
  void operator()(Self& self,
      const xio::error_code& ec = xio::error_code())
  {
    switch (state_)
    {
    case starting:
...
async_cancellable(CancellationFilter cancellation_filter,
    ...
    CompletionToken&& token)
{
  return xio::async_initiate<CompletionToken, void(bool)>(
      xio::composed<void(bool)>(
...
void compose_default_cancellation_test()
{
  ...
  xio::io_context ioc;
  xio::system_timer timer(ioc);
  xio::cancellation_signal signal;
  int count = 0;
  bool result = false;
  ...
  async_cancellable(default_filter(), timer,
      xio::bind_cancellation_slot(signal.slot(),
        bindns::bind(&compose_partial_cancellation_handler,
        &count, &result, _1)));

  // Total cancellation unsupported. Operation completes successfully.
  signal.emit(xio::cancellation_type::total);
  ...
  signal.emit(xio::cancellation_type::partial);
  ...
  signal.emit(xio::cancellation_type::terminal);
...
void compose_partial_cancellation_test()
{
  ...
  async_cancellable(xio::enable_partial_cancellation(), timer,
      bindns::bind(&compose_partial_cancellation_handler,
        &count, &result, _1));
...
void compose_total_cancellation_test()
{
  ...
  async_cancellable(xio::enable_total_cancellation(), timer,
      bindns::bind(&compose_partial_cancellation_handler,
        &count, &result, _1));
