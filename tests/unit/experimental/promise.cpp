...
void promise_tester()
{
  ...
  auto p2 = timer1.async_wait(
      xio::append(experimental::use_promise, 123));
...
struct test_cancel_impl_op
{
  ...
  template<typename Self>
  void operator()(Self& self, xio::error_code ec_)
  {
    ec = ec_;
    self.complete(ec_);
  }
...
{
  return xio::async_compose<CompletionToken, void(xio::error_code)>(
...
{
  xio::io_context ctx;
  xio::steady_timer tim{ctx, std::chrono::seconds(10)};
  xio::error_code ec;

  {
    auto p = test_cancel_impl(
        tim, ec, xio::experimental::use_promise);
  }
