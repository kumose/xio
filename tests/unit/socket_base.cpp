...
void test()
{
  ...
  socket_base::broadcast broadcast1(true);
  XIO_CHECK(broadcast1.value());
  XIO_CHECK(static_cast<bool>(broadcast1));
  XIO_CHECK(!!broadcast1);
  ...
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
  XIO_CHECK(broadcast2.value());
  XIO_CHECK(static_cast<bool>(broadcast2));
  XIO_CHECK(!!broadcast2);

  socket_base::broadcast broadcast3(false);
  XIO_CHECK(!broadcast3.value());
  XIO_CHECK(!static_cast<bool>(broadcast3));
  XIO_CHECK(!broadcast3);
  ...
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
  XIO_CHECK(!broadcast4.value());
  XIO_CHECK(!static_cast<bool>(broadcast4));
  XIO_CHECK(!broadcast4);

  // debug class.

  socket_base::debug debug1(true);
  XIO_CHECK(debug1.value());
  XIO_CHECK(static_cast<bool>(debug1));
  XIO_CHECK(!!debug1);
  udp_sock.set_option(debug1, ec);
#if defined(__linux__)
  // On Linux, only root can set SO_DEBUG.
  bool not_root = (ec == xio::error::access_denied);
  XIO_CHECK(!ec || not_root);
  XIO_WARN_MESSAGE(!ec, "Must be root to set debug socket option");
...
# endif // defined(XIO_WINDOWS) && defined(UNDER_CE)
  ...
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
# if defined(__linux__)
  XIO_CHECK(debug2.value() || not_root);
  XIO_CHECK(static_cast<bool>(debug2) || not_root);
  XIO_CHECK(!!debug2 || not_root);
# else // defined(__linux__)
  XIO_CHECK(debug2.value());
  XIO_CHECK(static_cast<bool>(debug2));
  XIO_CHECK(!!debug2);
# endif // defined(__linux__)
#endif // defined(XIO_WINDOWS) && defined(UNDER_CE)

  socket_base::debug debug3(false);
  XIO_CHECK(!debug3.value());
  XIO_CHECK(!static_cast<bool>(debug3));
  XIO_CHECK(!debug3);
...
# endif // defined(XIO_WINDOWS) && defined(UNDER_CE)
  ...
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
# if defined(__linux__)
  XIO_CHECK(!debug4.value() || not_root);
  XIO_CHECK(!static_cast<bool>(debug4) || not_root);
  XIO_CHECK(!debug4 || not_root);
# else // defined(__linux__)
  XIO_CHECK(!debug4.value());
  XIO_CHECK(!static_cast<bool>(debug4));
  XIO_CHECK(!debug4);
# endif // defined(__linux__)
#endif // defined(XIO_WINDOWS) && defined(UNDER_CE)
  ...
  socket_base::do_not_route do_not_route1(true);
  XIO_CHECK(do_not_route1.value());
  XIO_CHECK(static_cast<bool>(do_not_route1));
  XIO_CHECK(!!do_not_route1);
  ...
  XIO_CHECK_MESSAGE(!ec || do_not_route_unsupported,
      ec.value() << ", " << ec.message());
#endif // defined(XIO_WINDOWS) && defined(UNDER_CE)
  ...
  XIO_CHECK_MESSAGE(!ec || do_not_route_unsupported,
      ec.value() << ", " << ec.message());
  XIO_CHECK(do_not_route2.value() || do_not_route_unsupported);
  XIO_CHECK(static_cast<bool>(do_not_route2) || do_not_route_unsupported);
  XIO_CHECK(!!do_not_route2 || do_not_route_unsupported);
#endif // defined(XIO_WINDOWS) && defined(UNDER_CE)

  socket_base::do_not_route do_not_route3(false);
  XIO_CHECK(!do_not_route3.value());
  XIO_CHECK(!static_cast<bool>(do_not_route3));
  XIO_CHECK(!do_not_route3);
  ...
  XIO_CHECK_MESSAGE(!ec || do_not_route_unsupported,
      ec.value() << ", " << ec.message());
  XIO_CHECK(!do_not_route4.value() || do_not_route_unsupported);
  XIO_CHECK(!static_cast<bool>(do_not_route4) || do_not_route_unsupported);
  XIO_CHECK(!do_not_route4 || do_not_route_unsupported);
#endif // defined(XIO_WINDOWS) && defined(UNDER_CE)

  // keep_alive class.

  socket_base::keep_alive keep_alive1(true);
  XIO_CHECK(keep_alive1.value());
  XIO_CHECK(static_cast<bool>(keep_alive1));
  XIO_CHECK(!!keep_alive1);
  ...
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
  XIO_CHECK(keep_alive2.value());
  XIO_CHECK(static_cast<bool>(keep_alive2));
  XIO_CHECK(!!keep_alive2);

  socket_base::keep_alive keep_alive3(false);
  XIO_CHECK(!keep_alive3.value());
  XIO_CHECK(!static_cast<bool>(keep_alive3));
  XIO_CHECK(!keep_alive3);
  ...
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
  XIO_CHECK(!keep_alive4.value());
  XIO_CHECK(!static_cast<bool>(keep_alive4));
  XIO_CHECK(!keep_alive4);

  // send_buffer_size class.

  socket_base::send_buffer_size send_buffer_size1(4096);
  XIO_CHECK(send_buffer_size1.value() == 4096);
  ...
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
  XIO_CHECK(send_buffer_size2.value() == 4096);

  socket_base::send_buffer_size send_buffer_size3(16384);
  XIO_CHECK(send_buffer_size3.value() == 16384);
  ...
  XIO_CHECK(!!ec); // Not supported on Windows or Solaris.
#elif defined(__linux__)
  XIO_CHECK(!ec); // Not supported on Linux but can get value.
#else
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
  XIO_CHECK(send_low_watermark2.value() == 4096);
#endif

  socket_base::send_low_watermark send_low_watermark3(8192);
  XIO_CHECK(send_low_watermark3.value() == 8192);
...
#else
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
  XIO_CHECK(send_low_watermark4.value() == 8192);
#endif

  // receive_buffer_size class.

  socket_base::receive_buffer_size receive_buffer_size1(4096);
  XIO_CHECK(receive_buffer_size1.value() == 4096);
...
#if defined(XIO_WINDOWS) && defined(UNDER_CE)
  XIO_CHECK(!ec); // Not supported under Windows CE but can get value.
#else // defined(XIO_WINDOWS) && defined(UNDER_CE)
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
  XIO_CHECK(receive_buffer_size2.value() == 4096);
#endif // defined(XIO_WINDOWS) && defined(UNDER_CE)

  socket_base::receive_buffer_size receive_buffer_size3(16384);
  XIO_CHECK(receive_buffer_size3.value() == 16384);
...
#else // defined(XIO_WINDOWS) && defined(UNDER_CE)
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
  XIO_CHECK(receive_buffer_size4.value() == 16384);
#endif // defined(XIO_WINDOWS) && defined(UNDER_CE)

  // receive_low_watermark class.

  socket_base::receive_low_watermark receive_low_watermark1(4096);
  XIO_CHECK(receive_low_watermark1.value() == 4096);
...
#else
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
  XIO_CHECK(receive_low_watermark2.value() == 4096);
#endif

  socket_base::receive_low_watermark receive_low_watermark3(8192);
  XIO_CHECK(receive_low_watermark3.value() == 8192);
...
#else
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
  XIO_CHECK(receive_low_watermark4.value() == 8192);
#endif

  // reuse_address class.

  socket_base::reuse_address reuse_address1(true);
  XIO_CHECK(reuse_address1.value());
  XIO_CHECK(static_cast<bool>(reuse_address1));
  XIO_CHECK(!!reuse_address1);
  ...
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
  XIO_CHECK(reuse_address2.value());
  XIO_CHECK(static_cast<bool>(reuse_address2));
  XIO_CHECK(!!reuse_address2);

  socket_base::reuse_address reuse_address3(false);
  XIO_CHECK(!reuse_address3.value());
  XIO_CHECK(!static_cast<bool>(reuse_address3));
  XIO_CHECK(!reuse_address3);
  ...
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
  XIO_CHECK(!reuse_address4.value());
  XIO_CHECK(!static_cast<bool>(reuse_address4));
  XIO_CHECK(!reuse_address4);

  // linger class.

  socket_base::linger linger1(true, 60);
  XIO_CHECK(linger1.enabled());
  XIO_CHECK(linger1.timeout() == 60);
  ...
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
  XIO_CHECK(linger2.enabled());
  XIO_CHECK(linger2.timeout() == 60);

  socket_base::linger linger3(false, 0);
  XIO_CHECK(!linger3.enabled());
  XIO_CHECK(linger3.timeout() == 0);
  ...
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
  XIO_CHECK(!linger4.enabled());

  // enable_connection_aborted class.

  socket_base::enable_connection_aborted enable_connection_aborted1(true);
  XIO_CHECK(enable_connection_aborted1.value());
  XIO_CHECK(static_cast<bool>(enable_connection_aborted1));
  XIO_CHECK(!!enable_connection_aborted1);
  ...
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
  XIO_CHECK(enable_connection_aborted2.value());
  XIO_CHECK(static_cast<bool>(enable_connection_aborted2));
  XIO_CHECK(!!enable_connection_aborted2);

  socket_base::enable_connection_aborted enable_connection_aborted3(false);
  XIO_CHECK(!enable_connection_aborted3.value());
  XIO_CHECK(!static_cast<bool>(enable_connection_aborted3));
  XIO_CHECK(!enable_connection_aborted3);
  ...
  XIO_CHECK_MESSAGE(!ec, ec.value() << ", " << ec.message());
  XIO_CHECK(!enable_connection_aborted4.value());
  XIO_CHECK(!static_cast<bool>(enable_connection_aborted4));
  XIO_CHECK(!enable_connection_aborted4);
...
(
  "socket_base",
  XIO_COMPILE_TEST_CASE(socket_base_compile::test)
  XIO_TEST_CASE(socket_base_runtime::test)
)
