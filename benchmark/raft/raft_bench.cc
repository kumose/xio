...
int init_raft(server_stuff& stuff) {
    ...
    xio_opt.thread_pool_size_ = 32;
    stuff.xio_svc_ = cs_new<xio_service>(xio_opt);

    stuff.xio_listener_ =
        stuff.xio_svc_->create_rpc_listener( stuff.port_ );
    ptr<delayed_task_scheduler> scheduler = stuff.xio_svc_;
    ptr<rpc_client_factory> rpc_cli_factory = stuff.xio_svc_;
    ...
    // Listen.
    stuff.xio_listener_->listen( stuff.raft_instance_ );
